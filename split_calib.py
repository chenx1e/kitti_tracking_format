...
def main(root, mode, output_dir):
    ...
    calib_path = os.path.join(root, mode, 'calib')
    ...
    for seq in seq_list:
        ...
        calib_files = os.path.join(calib_path, seq+'.txt')

        # each frame
        for file in img_files:
            file_name = int(file.split('.')[0])
            correct_file_name = file_name + start_frame
            correct_file_name = str(correct_file_name).zfill(6) + '.txt'

            source_path = calib_files
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(root, mode + '_for_detection', 'calib')
